...
file = './A1_datasets/microchips.csv'
features = [0, 1] # X
label = 2         # y
...
fig.suptitle("Classification using Scikit's KNN Classifier")
current_row = 0

# Unknown chips
chip1 = np.array([-0.3,  1.0]) # Fail
chip2 = np.array([-0.5, -0.1]) # OK
chip3 = np.array([ 0.6,  0.0]) # OK
...
for k in K:
    ...
    accuracy_score = knn.score(X_train, y_train)
    ...
    pred_chip1 = knn.predict(chip1.reshape(1, -1))
    pred_chip2 = knn.predict(chip2.reshape(1, -1))
    pred_chip3 = knn.predict(chip3.reshape(1, -1))
    chip_results = np.array([pred_chip1, pred_chip2, pred_chip3])
    # chip_results = knn.predict(np.array([chip1, chip2, chip3]))
